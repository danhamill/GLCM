...
def glcm_calc(im,segments_slic):
    ...
    for k in np.unique(segments_slic):
       ...
       if im_count > 0.75*count:
           ...
           Z,ind = sliding_window(cim,(win,win),(win,win))
           
           w = Parallel(n_jobs = 1, verbose=0)(delayed(glcm_calc2)(Z[k], win) for k in xrange(len(Z)))
...
def glcm_calc2(Z,win):
    if np.count_nonzero(Z) > 0.95*win**2:  
