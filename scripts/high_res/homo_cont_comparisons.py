...
for n in xrange(len(homo_list)):
    ...
    merge = merge[['homo_mean','cont_mean','substrate_x','homo_25','cont_25','homo_75','cont_75']]
    ...
    oName = root + os.sep + "homo_contrast_comparison_" + meter +".png"   
    fig,axes = plt.subplots(figsize =(8,6),nrows=3,ncols=2)

    merge.query('substrate == "sand"').plot.scatter(ax =axes[0,0], x='homo_25',y='cont_25',color='blue')
    cent_df.query('index == "sand"').plot.scatter(ax = axes[0,0], x='x_cent',y='y_cent', yerr= 'y_err', xerr='x_err',color='blue',s=100)
    merge.query('substrate == "gravel"').plot.scatter(ax =axes[0,0], x='homo_25',y='cont_25',color='green')
    ...
    merge.query('substrate == "boulders"').plot.scatter(ax =axes[0,0],x='homo_25',y='cont_25',color='red')
    ...
    cent_df.loc['boulders'] = pd.Series({'x_cent':b_centroid[0] ,'y_cent':b_centroid[1] ,'y_err': b_err[1],'x_err':b_err[0]})
    cent_df = cent_df.reset_index()
    merge.query('substrate == "sand"').plot.scatter(ax =axes[1,0], x='homo_mean',y='cont_mean',color='blue')
    cent_df.query('index == "sand"').plot.scatter(ax = axes[1,0], x='x_cent',y='y_cent', yerr= 'y_err', xerr='x_err',color='blue',s=100)
    merge.query('substrate == "gravel"').plot.scatter(ax =axes[1,0], x='homo_mean',y='cont_mean',color='green')
    ...
    merge.query('substrate == "boulders"').plot.scatter(ax =axes[1,0],x='homo_mean',y='cont_mean',color='red')
    ...
    cent_df = cent_df.reset_index()
    merge.query('substrate == "sand"').plot.scatter(ax =axes[2,0], x='homo_75',y='cont_75',color='blue')
    cent_df.query('index == "sand"').plot.scatter(ax = axes[2,0], x='x_cent',y='y_cent', yerr= 'y_err', xerr='x_err',color='blue',s=100)
    
    merge.query('substrate == "gravel"').plot.scatter(ax =axes[2,0], x='homo_75',y='cont_75',color='green')
    ...
    merge.query('substrate == "boulders"').plot.scatter(ax =axes[2,0],x='homo_75',y='cont_75',color='red')
