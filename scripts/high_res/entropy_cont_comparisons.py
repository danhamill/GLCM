...
def centeroidnp(df,query1,metric):
    length = df.query(query1)['entropy_'+ metric].dropna().values.size
    sum_x = np.nansum(df.query(query1)['entropy_'+ metric].values)
    sum_y = np.nansum(df.query(query1)['cont_' +  metric].values)
    return sum_x/length, sum_y/length
    
def error_bars(df,query1,metric):
    y_err = np.nanstd(df.query(query1)['cont_' + metric].values)
    x_err = np.nanstd(df.query(query1)['entropy_'+ metric].values)
    return x_err, y_err
...
for n in xrange(len(ent_list)):
    ...
    df1 = pd.read_csv(ent,sep=',')
    df2 = pd.read_csv(cont,sep=',')
    df1.rename(columns={'max':'entropy_max', 'mean':'entropy_mean', 'median':'entropy_median','min':'entropy_min','percentile_25':'entropy_25','percentile_50':'entropy_50', 'percentile_75':'entropy_75','std':'entropy_std'},inplace=True)   
    df2.rename(columns={'max':'cont_max', 'mean':'cont_mean', 'median':'cont_median','min':'cont_min','percentile_25':'cont_25','percentile_50':'cont_50', 'percentile_75':'cont_75','std':'cont_std'},inplace=True)   
    
    
    merge =df1.merge(df2,left_index=True, right_index=True, how='left')
    
    merge = merge[['entropy_mean','cont_mean','substrate_x','entropy_25','cont_25','entropy_75','cont_75']]
    ...
    oName = root + os.sep + "entropy_contrast_comparison_" + meter +".png"   
    fig,axes = plt.subplots(figsize =(8,6),nrows=3,ncols=2)

    merge.query('substrate == "sand"').plot.scatter(ax =axes[0,0], x='entropy_25',y='cont_25',color='blue')
    cent_df.query('index == "sand"').plot.scatter(ax = axes[0,0], x='x_cent',y='y_cent', yerr= 'y_err', xerr='x_err',color='blue',s=100)
    merge.query('substrate == "gravel"').plot.scatter(ax =axes[0,0], x='entropy_25',y='cont_25',color='green')
    ...
    merge.query('substrate == "boulders"').plot.scatter(ax =axes[0,0],x='entropy_25',y='cont_25',color='red')
    ...
    cent_df.loc['boulders'] = pd.Series({'x_cent':b_centroid[0] ,'y_cent':b_centroid[1] ,'y_err': b_err[1],'x_err':b_err[0]})
    cent_df = cent_df.reset_index()
    merge.query('substrate == "sand"').plot.scatter(ax =axes[1,0], x='entropy_mean',y='cont_mean',color='blue')
    cent_df.query('index == "sand"').plot.scatter(ax = axes[1,0], x='x_cent',y='y_cent', yerr= 'y_err', xerr='x_err',color='blue',s=100)
    merge.query('substrate == "gravel"').plot.scatter(ax =axes[1,0], x='entropy_mean',y='cont_mean',color='green')
    ...
    merge.query('substrate == "boulders"').plot.scatter(ax =axes[1,0],x='entropy_mean',y='cont_mean',color='red')
    ...
    cent_df = cent_df.reset_index()
    merge.query('substrate == "sand"').plot.scatter(ax =axes[2,0], x='entropy_75',y='cont_75',color='blue')
    cent_df.query('index == "sand"').plot.scatter(ax = axes[2,0], x='x_cent',y='y_cent', yerr= 'y_err', xerr='x_err',color='blue',s=100)
    
    merge.query('substrate == "gravel"').plot.scatter(ax =axes[2,0], x='entropy_75',y='cont_75',color='green')
    ...
    merge.query('substrate == "boulders"').plot.scatter(ax =axes[2,0],x='entropy_75',y='cont_75',color='red')
