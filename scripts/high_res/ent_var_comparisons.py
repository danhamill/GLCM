...
def centeroidnp(df,query1,metric):
    length = df.query(query1)['ent_'+ metric].dropna().values.size
    sum_x = np.nansum(df.query(query1)['ent_'+ metric].values)
...
def error_bars(df,query1,metric):
    ...
    x_err = np.nanstd(df.query(query1)['ent_'+ metric].values)
    return x_err, y_err
...
#n=3
for n in xrange(len(entropy_list)):
    ...
    df1.rename(columns={'max':'ent_max', 'mean':'ent_mean', 'median':'ent_median','min':'ent_min','percentile_25':'ent_25','percentile_50':'ent_50', 'percentile_75':'ent_75','std':'ent_std'},inplace=True)   
    ...
    merge = merge[['ent_mean','var_mean','substrate_x','ent_25','var_25','ent_75','var_75']]
    ...
    oName = root + os.sep + "ent_variance_comparison_expanded_" + meter +".png"   
    fig,axes = plt.subplots(figsize =(8,6),nrows=3,ncols=2)
    
    merge.query('substrate == "sand"').plot.scatter(ax =axes[0,0], x='ent_25',y='var_25',color='blue')
    cent_df.query('index == "sand"').plot.scatter(ax = axes[0,0], x='x_cent',y='y_cent', yerr= 'y_err', xerr='x_err',color='blue',s=100)
    merge.query('substrate == "gravel"').plot.scatter(ax =axes[0,0], x='ent_25',y='var_25',color='green')
    ...
    merge.query('substrate == "boulders"').plot.scatter(ax =axes[0,0],x='ent_25',y='var_25',color='red')
    ...
    cent_df.loc['boulders'] = pd.Series({'x_cent':b_centroid[0] ,'y_cent':b_centroid[1] ,'y_err': b_err[1],'x_err':b_err[0]})
    cent_df = cent_df.reset_index()
    merge.query('substrate == "sand"').plot.scatter(ax =axes[1,0], x='ent_mean',y='var_mean',color='blue')
    cent_df.query('index == "sand"').plot.scatter(ax = axes[1,0], x='x_cent',y='y_cent', yerr= 'y_err', xerr='x_err',color='blue',s=100)
    merge.query('substrate == "gravel"').plot.scatter(ax =axes[1,0], x='ent_mean',y='var_mean',color='green')
    ...
    merge.query('substrate == "boulders"').plot.scatter(ax =axes[1,0],x='ent_mean',y='var_mean',color='red')
    ...
    cent_df = cent_df.reset_index()
    merge.query('substrate == "sand"').plot.scatter(ax =axes[2,0], x='ent_75',y='var_75',color='blue')
    cent_df.query('index == "sand"').plot.scatter(ax = axes[2,0], x='x_cent',y='y_cent', yerr= 'y_err', xerr='x_err',color='blue',s=100)
    
    merge.query('substrate == "gravel"').plot.scatter(ax =axes[2,0], x='ent_75',y='var_75',color='green')
    ...
    merge.query('substrate == "boulders"').plot.scatter(ax =axes[2,0],x='ent_75',y='var_75',color='red')
