...
def centeroidnp(df,query1,metric):
    length = df.query(query1)['var_'+ metric].dropna().values.size
    sum_x = np.nansum(df.query(query1)['var_'+ metric].values)
...
def error_bars(df,query1,metric):
    ...
    x_err = np.nanstd(df.query(query1)['var_'+ metric].values)
    return x_err, y_err
...
for n in xrange(len(var_list)):
    
    var = var_list[n]
    cont = cont_list[n]
    meter = var.split('\\')[-1].split('_')[1]
    
    df1 = pd.read_csv(var,sep=',')
    df2 = pd.read_csv(cont,sep=',')
    df1.rename(columns={'max':'var_max', 'mean':'var_mean', 'median':'var_median','min':'var_min','percentile_25':'var_25','percentile_50':'var_50', 'percentile_75':'var_75','std':'var_std'},inplace=True)   
    ...
    merge = merge[['var_mean','cont_mean','substrate_x','var_25','cont_25','var_75','cont_75']]
    ...
    oName = root + os.sep + "variance_contrast_comparison_" + meter +".png"   
    fig,axes = plt.subplots(figsize =(8,6),nrows=3,ncols=2)

    merge.query('substrate == "sand"').plot.scatter(ax =axes[0,0], x='var_25',y='cont_25',color='blue')
    cent_df.query('index == "sand"').plot.scatter(ax = axes[0,0], x='x_cent',y='y_cent', yerr= 'y_err', xerr='x_err',color='blue',s=100)
    merge.query('substrate == "gravel"').plot.scatter(ax =axes[0,0], x='var_25',y='cont_25',color='green')
    ...
    merge.query('substrate == "boulders"').plot.scatter(ax =axes[0,0],x='var_25',y='cont_25',color='red')
    ...
    cent_df.loc['boulders'] = pd.Series({'x_cent':b_centroid[0] ,'y_cent':b_centroid[1] ,'y_err': b_err[1],'x_err':b_err[0]})
    cent_df = cent_df.reset_index()
    merge.query('substrate == "sand"').plot.scatter(ax =axes[1,0], x='var_mean',y='cont_mean',color='blue')
    cent_df.query('index == "sand"').plot.scatter(ax = axes[1,0], x='x_cent',y='y_cent', yerr= 'y_err', xerr='x_err',color='blue',s=100)
    merge.query('substrate == "gravel"').plot.scatter(ax =axes[1,0], x='var_mean',y='cont_mean',color='green')
    ...
    merge.query('substrate == "boulders"').plot.scatter(ax =axes[1,0],x='var_mean',y='cont_mean',color='red')
    ...
    cent_df = cent_df.reset_index()
    merge.query('substrate == "sand"').plot.scatter(ax =axes[2,0], x='var_75',y='cont_75',color='blue')
    cent_df.query('index == "sand"').plot.scatter(ax = axes[2,0], x='x_cent',y='y_cent', yerr= 'y_err', xerr='x_err',color='blue',s=100)
    
    merge.query('substrate == "gravel"').plot.scatter(ax =axes[2,0], x='var_75',y='cont_75',color='green')
    ...
    merge.query('substrate == "boulders"').plot.scatter(ax =axes[2,0],x='var_75',y='cont_75',color='red')
